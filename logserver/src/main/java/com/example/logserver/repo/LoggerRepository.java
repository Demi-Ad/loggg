...
public interface LoggerRepository extends JpaRepository<Logger, Long> {
    ...
    @Query(value = """
            select new com.example.logserver.service.dto.LoggerResponse(l.service, l.createdAt, (select count (*) from Log lo where l = lo.logger))
            from Logger l
            order by l.createdAt asc
            """,
    countQuery = "select count (*) from Logger")
    Page<LoggerResponse> findAllToResponse(Pageable pageable);
